import { Dt as mergeProps, G as createBlock, Jt as renderSlot, K as createCommentVNode, Vt as openBlock, W as createBaseVNode, Yt as resolveComponent, Zt as resolveDynamicComponent, an as toHandlers, ar as normalizeClass, et as createVNode, q as createElementBlock, sr as normalizeStyle } from "./vue.runtime.esm-bundler-CKUwx75R.js";
import { H as pt, J as f, O as Mt, _ as l, b as s, t as BaseStyle } from "./style-zV7_3CTQ.js";
import "./basecomponent-CyZyZ_FO.js";
import { t as script$3 } from "./baseicon-I8adJg6o.js";
import { t as script$4 } from "./times-DD9JzGTY.js";
import { t as script$5 } from "./angledown-DNOTvv54.js";
import { n as script$6, t as script$7 } from "./inputtext-Bez7Z9VD.js";
...
var InputNumberStyle = BaseStyle.extend({
	...
	classes: {
		...
		incrementButton: function incrementButton(_ref2) {
			...
			return ["p-inputnumber-button p-inputnumber-increment-button", { "p-disabled": props.showButtons && props.max !== null && instance.maxBoundry() }];
		},
		decrementButton: function decrementButton(_ref3) {
			var instance = _ref3.instance, props = _ref3.props;
			return ["p-inputnumber-button p-inputnumber-decrement-button", { "p-disabled": props.showButtons && props.min !== null && instance.minBoundry() }];
		}
	}
});
//#endregion
//#region node_modules/primevue/inputnumber/index.mjs
var script$1 = {
...
var script = {
	...
	watch: {
		d_value: {
			...
			handler: function handler(newValue) {
				...
				if ((_this$$refs$clearIcon = this.$refs.clearIcon) !== null && _this$$refs$clearIcon !== void 0 && (_this$$refs$clearIcon = _this$$refs$clearIcon.$el) !== null && _this$$refs$clearIcon !== void 0 && _this$$refs$clearIcon.style) this.$refs.clearIcon.$el.style.display = l(newValue) ? "none" : "block";
			}
		},
	...
	mounted: function mounted() {
		...
		if ((_this$$refs$clearIcon2 = this.$refs.clearIcon) !== null && _this$$refs$clearIcon2 !== void 0 && (_this$$refs$clearIcon2 = _this$$refs$clearIcon2.$el) !== null && _this$$refs$clearIcon2 !== void 0 && _this$$refs$clearIcon2.style) this.$refs.clearIcon.$el.style.display = !this.$filled ? "none" : "block";
	},
	methods: {
		...
		constructParser: function constructParser() {
			...
			var numerals = _toConsumableArray(new Intl.NumberFormat(this.locale, { useGrouping: false }).format(9876543210)).reverse();
		...
		updateConstructParser: function updateConstructParser(newValue, oldValue) {
			if (newValue !== oldValue) this.constructParser();
		},
		escapeRegExp: function escapeRegExp(text) {
			return text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
		},
		getDecimalExpression: function getDecimalExpression() {
			var formatter = new Intl.NumberFormat(this.locale, _objectSpread(_objectSpread({}, this.getOptions()), {}, { useGrouping: false }));
			var formatted = formatter.format(1.1);
			if (formatted === formatter.format(1)) return /* @__PURE__ */ new RegExp("[]", "g");
			return new RegExp("[".concat(formatted.replace(this._currency, "").trim().replace(this._numeral, ""), "]"), "g");
		},
		getGroupingExpression: function getGroupingExpression() {
			this.groupChar = new Intl.NumberFormat(this.locale, { useGrouping: true }).format(1e6).trim().replace(this._numeral, "").charAt(0);
		...
		getPrefixExpression: function getPrefixExpression() {
			...
			else this.prefixChar = new Intl.NumberFormat(this.locale, {
				style: this.mode,
				currency: this.currency,
				currencyDisplay: this.currencyDisplay
			}).format(1).split("1")[0];
		...
		getSuffixExpression: function getSuffixExpression() {
			...
			else this.suffixChar = new Intl.NumberFormat(this.locale, {
				style: this.mode,
				currency: this.currency,
				currencyDisplay: this.currencyDisplay,
				minimumFractionDigits: 0,
		...
		formatValue: function formatValue(value) {
			if (value != null) {
				...
				if (this.format) {
					var formattedValue = new Intl.NumberFormat(this.locale, this.getOptions()).format(value);
					if (this.prefix) formattedValue = this.prefix + formattedValue;
					if (this.suffix) formattedValue = formattedValue + this.suffix;
		...
		onUpButtonKeyDown: function onUpButtonKeyDown(event) {
			if (event.code === "Space" || event.code === "Enter" || event.code === "NumpadEnter") this.repeat(event, null, 1);
		},
		...
		onUserInput: function onUserInput() {
			if (this.isSpecialChar) this.$refs.input.$el.value = this.lastValue;
		...
		onInputKeyDown: function onInputKeyDown(event) {
			...
			switch (event.code || event.key) {
				...
				case "ArrowLeft":
					...
					} else if (!this.isNumeralChar(inputValue.charAt(selectionStart - 1))) event.preventDefault();
					break;
				case "ArrowRight":
					if (selectionRange > 1) {
						var _cursorPosition = selectionEnd - 1;
						this.$refs.input.$el.setSelectionRange(_cursorPosition, _cursorPosition);
					} else if (!this.isNumeralChar(inputValue.charAt(selectionStart))) event.preventDefault();
					break;
				case "Tab":
				case "Enter":
				case "NumpadEnter":
					...
					if (selectionStart === selectionEnd) {
						...
						if (this.isNumeralChar(deleteChar)) {
							...
							if (this._group.test(deleteChar)) {
								...
								if (decimalLength) this.$refs.input.$el.setSelectionRange(selectionStart - 1, selectionStart - 1);
					...
					if (selectionStart === selectionEnd) {
						...
						if (this.isNumeralChar(_deleteChar)) {
							...
							if (this._group.test(_deleteChar)) {
								...
								this._decimal.lastIndex = 0;
								if (_decimalLength) this.$refs.input.$el.setSelectionRange(selectionStart + 1, selectionStart + 1);
				...
				case "Home":
					event.preventDefault();
					if (s(this.min)) this.updateModel(event, this.min);
					break;
				case "End":
					event.preventDefault();
					if (s(this.max)) this.updateModel(event, this.max);
					break;
			}
		},
		onInputKeyPress: function onInputKeyPress(event) {
			...
			if (Number(_char) >= 0 && Number(_char) <= 9 || isMinusSign || isDecimalSign) this.insert(event, _char, {
				isDecimalSign,
				isMinusSign
			});
		},
		...
		getDecimalCharIndexes: function getDecimalCharIndexes(val) {
			...
			this._decimal.lastIndex = 0;
			var decimalCharIndexWithoutPrefix = val.replace(this._prefix, "").trim().replace(/\s/g, "").replace(this._currency, "").search(this._decimal);
			this._decimal.lastIndex = 0;
			return {
				decimalCharIndex,
		...
		insert: function insert(event, text) {
			...
			if (sign.isMinusSign) {
				...
				if (selectionStart === 0 || selectionStart === currencyCharIndex + 1) {
					...
					if (isNewMinusSign || selectionEnd !== 0) newValueStr = this.insertText(inputValue, text, 0, selectionEnd);
		...
		initCursor: function initCursor() {
			...
			if (this.isNumeralChar(_char4)) return selectionStart + prefixLength;
			...
			if (index !== null) this.$refs.input.$el.setSelectionRange(index + 1, index + 1);
			else {
				i = selectionStart;
				while (i < valueLength) {
					_char4 = inputValue.charAt(i);
				...
				if (index !== null) this.$refs.input.$el.setSelectionRange(index, index);
			}
			return index || 0;
		},
		onInputClick: function onInputClick() {
			var currentValue = this.$refs.input.$el.value;
			if (!this.readonly && currentValue !== Mt()) this.initCursor();
		},
		...
		updateInput: function updateInput(value, insertedValueStr, operation, valueStr) {
			...
			if (currentLength === 0) {
				...
				var selectionEnd = this.initCursor() + insertedValueStr.length;
				...
				if (operation === "range-insert") {
					...
					var startExpr = (startValue !== null ? startValue.toString() : "").split("").join("(".concat(this.groupChar, ")?"));
					...
					if (operation === "insert" || operation === "delete-back-single") this.$refs.input.$el.setSelectionRange(_selectionEnd + 1, _selectionEnd + 1);
					else if (operation === "delete-single") this.$refs.input.$el.setSelectionRange(_selectionEnd - 1, _selectionEnd - 1);
					else if (operation === "delete-range" || operation === "spin") this.$refs.input.$el.setSelectionRange(_selectionEnd, _selectionEnd);
				} else if (operation === "delete-back-single") {
					var prevChar = inputValue.charAt(_selectionEnd - 1);
					var nextChar = inputValue.charAt(_selectionEnd);
					var diff = currentLength - newLength;
					var isGroupChar = this._group.test(nextChar);
					if (isGroupChar && diff === 1) _selectionEnd += 1;
					else if (!isGroupChar && this.isNumeralChar(prevChar)) _selectionEnd += -1 * diff + 1;
					this._group.lastIndex = 0;
					this.$refs.input.$el.setSelectionRange(_selectionEnd, _selectionEnd);
				} else if (inputValue === "-" && operation === "insert") {
					this.$refs.input.$el.setSelectionRange(0, 0);
					var _selectionEnd2 = this.initCursor() + insertedValueStr.length + 1;
			...
			if ((_this$$refs$clearIcon3 = this.$refs.clearIcon) !== null && _this$$refs$clearIcon3 !== void 0 && (_this$$refs$clearIcon3 = _this$$refs$clearIcon3.$el) !== null && _this$$refs$clearIcon3 !== void 0 && _this$$refs$clearIcon3.style) this.$refs.clearIcon.$el.style.display = l(newValue) ? "none" : "block";
		},
		...
		onInputFocus: function onInputFocus(event) {
			this.focused = true;
			if (!this.disabled && !this.readonly && this.$refs.input.$el.value !== Mt() && this.highlightOnFocus) event.target.select();
		...
		onInputBlur: function onInputBlur(event) {
			...
			this.updateModel(event, newValue);
			if (!this.disabled && !this.readonly && this.highlightOnFocus) pt();
		},
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createElementBlock("span", mergeProps({ "class": _ctx.cx("root") }, _ctx.ptmi("root"), { "data-p": $options.dataP }), [
		...
		_ctx.showClear && _ctx.buttonLayout !== "vertical" ? renderSlot(_ctx.$slots, "clearicon", {
			key: 0,
		...
		_ctx.showButtons && _ctx.buttonLayout === "stacked" ? (openBlock(), createElementBlock("span", mergeProps({
		...
		}, _ctx.ptm("buttonGroup"), { "data-p": $options.dataP }), [renderSlot(_ctx.$slots, "incrementbutton", { listeners: $options.upButtonListeners }, function() {
			return [createBaseVNode("button", mergeProps({ "class": [_ctx.cx("incrementButton"), _ctx.incrementButtonClass] }, toHandlers($options.upButtonListeners, true), {
				disabled: _ctx.disabled,
				tabindex: -1,
				"aria-hidden": "true",
				type: "button"
			}, _ctx.ptm("incrementButton"), { "data-p": $options.dataP }), [renderSlot(_ctx.$slots, _ctx.$slots.incrementicon ? "incrementicon" : "incrementbuttonicon", {}, function() {
			...
			return [createBaseVNode("button", mergeProps({ "class": [_ctx.cx("decrementButton"), _ctx.decrementButtonClass] }, toHandlers($options.downButtonListeners, true), {
				disabled: _ctx.disabled,
				tabindex: -1,
				"aria-hidden": "true",
				type: "button"
			}, _ctx.ptm("decrementButton"), { "data-p": $options.dataP }), [renderSlot(_ctx.$slots, _ctx.$slots.decrementicon ? "decrementicon" : "decrementbuttonicon", {}, function() {
...
//# sourceMappingURL=primevue_inputnumber.js.map