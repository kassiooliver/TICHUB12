...
export class CartRepository {
  async getCartByUserId(userId: string): Promise<CartItemEntity[]> {
    const result = await pool.query<CartItemRow>(
      `SELECT ci.quantity, p.id, p.name, p.price, p.stock, p.category_id
       FROM cart_items ci
    ...
    const result = await pool.query<CartItemRow>(
      `INSERT INTO cart_items (user_id, product_id, quantity)
       ...
       RETURNING quantity,
         ...
         (SELECT category_id FROM products WHERE id = $2) AS category_id`,
  ...
  async updateItem(userId: string, productId: string, quantity: number): Promise<CartItemEntity | null> {
    const result = await pool.query<CartItemRow>(
      `UPDATE cart_items ci
       SET quantity = $3
       FROM products p
       WHERE ci.product_id = p.id AND ci.user_id = $1 AND ci.product_id = $2
       RETURNING ci.quantity, p.id, p.name, p.price, p.stock, p.category_id`,
      [userId, productId, quantity]
    )
  ...
  private toProduct(row: CartItemRow) {
